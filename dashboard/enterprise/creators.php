...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['creator_id'])) {
    ...
    $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM enterprise_creators WHERE enterprise_id = ? AND creator_id = ?");
    $checkStmt->execute([$userId, $creatorId]);

    if ($checkStmt->fetchColumn() == 0) {
        $insert = $pdo->prepare("INSERT INTO enterprise_creators (enterprise_id, creator_id, status, requested_at) VALUES (?, ?, 'Pending', NOW())");
...
$requestedStmt = $pdo->prepare("SELECT creator_id FROM enterprise_creators WHERE enterprise_id = ?");
