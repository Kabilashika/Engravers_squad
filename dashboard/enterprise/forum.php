...
$pageTitle = "Enterprise Forum - Growth Grid";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_post'])) {
    ...
    if ($title && $content) {
        $stmt = $pdo->prepare("INSERT INTO enterprise_forum_posts (enterprise_id, title, content, created_at) VALUES (?, ?, ?, NOW())");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_comment'])) {
    ...
    if ($comment) {
        $stmt = $pdo->prepare("INSERT INTO enterprise_forum_comments (post_id, enterprise_id, comment, created_at) VALUES (?, ?, ?, NOW())");
...
$posts = $pdo->query("
    SELECT p.*, u.username 
    FROM enterprise_forum_posts p
    JOIN users u ON p.enterprise_id = u.id
...
$comments = $pdo->query("
    SELECT c.*, u.username 
    FROM enterprise_forum_comments c
    JOIN users u ON c.enterprise_id = u.id
...
<main class="page-wrapper">
    <section style="max-width: 1000px; margin: auto; padding: 40px 20px; line-height: 1.6;">
        <h2 style="text-align: center; color: #f0c330;">Enterprise Forum</h2>

        <!-- Post Form -->
        <div style="margin-bottom: 40px; background: #f4f4f4; padding: 20px; border-radius: 10px;">
            <h3 style="color: #003366;">Start a New Discussion</h3>
            <form method="POST">
                ...
                <textarea name="content" placeholder="What would you like to share?" rows="4" required style="width: 100%; padding: 10px; border-radius: 6px;"></textarea>
        ...
        <?php if (empty($posts)): ?>
            <p style="text-align: center; color: #000000ff;">No forum posts available yet.</p>
        <?php else: ?>
