...
// Redirect if not logged in or not a enterprise user
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'enterprise') {
    header("Location: ../../login.php");
    exit;
}

$pageTitle = "Enterprise Dashboard - Growth Grid";
...
$serviceStmt = $pdo->prepare("SELECT COUNT(*) FROM enterprise_services WHERE enterprise_id = ?");
$serviceStmt->execute([$userId]);
$serviceCount = $serviceStmt->fetchColumn();

// Count of enrolled courses
$courseStmt = $pdo->prepare("SELECT COUNT(*) FROM enterprise_enrollments WHERE enterprise_id = ?");
$courseStmt->execute([$userId]);
$courseCount = $courseStmt->fetchColumn();

// Count of connected advisors
$advisorStmt = $pdo->prepare("SELECT COUNT(*) FROM enterprise_advisors WHERE enterprise_id = ?");
$advisorStmt->execute([$userId]);
$advisorCount = $advisorStmt->fetchColumn();

// Count of reviews made by the enterprise
