...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['course_id'])) {
    ...
    if ($_POST['action'] === 'enroll') {
        ...
        if (!$alreadyEnrolled) {
            $enroll = $pdo->prepare("INSERT INTO enrollments (user_id, course_id, progress) VALUES (?, ?, 0)");
        ...
        $unenroll = $pdo->prepare("DELETE FROM enrollments WHERE user_id = ? AND course_id = ?");
...
$enrolledStmt = $pdo->prepare("SELECT course_id FROM enrollments WHERE user_id = ?");
...
<main class="page-wrapper">
    <section style="max-width: 1000px; margin: auto; padding: 40px 20px;">
        <h2 style="text-align:center; color: #f0c330; font-size: 30px;">Available Courses</h2>
        ...
        <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 25px; margin-top: 30px;">
            <?php foreach ($courses as $course): ?>
                <div style="border: 1px solid #ccc; border-radius: 10px; padding: 20px; width: 280px; box-shadow: 0 0 10px #eee; background: #fff; display: flex; flex-direction: column; align-items: center;">
                    <?php
                        $imgPath = !empty($course['course_image']) 
                            ? '/digital_platform/uploads/course_images/' . $course['course_image']
                            : '/digital_platform/uploads/course_images/tools.webp';
                    ?>
                    <img src="<?php echo $imgPath; ?>" alt="Course Image" style="width: 220px; height: 130px; object-fit: cover; border-radius: 8px; margin-bottom: 15px;">
                    <h3 style="color: #003366; text-align: center; margin-bottom: 8px;"><?php echo htmlspecialchars($course['title']); ?></h3>
                    <p style="font-size: 14px; margin-bottom: 8px; text-align: center;"><?php echo htmlspecialchars($course['description']); ?></p>
