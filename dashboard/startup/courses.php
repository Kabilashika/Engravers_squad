...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['course_id'])) {
    ...
    if ($_POST['action'] === 'enroll') {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM startup_enrollments WHERE startup_id = ? AND course_id = ?");
        $stmt->execute([$userId, $courseId]);
        $alreadyEnrolled = $stmt->fetchColumn();

        if (!$alreadyEnrolled) {
            $enroll = $pdo->prepare("INSERT INTO startup_enrollments (startup_id, course_id, progress) VALUES (?, ?, 0)");
        ...
        $unenroll = $pdo->prepare("DELETE FROM startup_enrollments WHERE startup_id = ? AND course_id = ?");
...
$enrolledStmt = $pdo->prepare("SELECT course_id FROM startup_enrollments WHERE startup_id = ?");
