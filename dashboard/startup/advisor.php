...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['advisor_id'])) {
    ...
    if ($_POST['action'] === 'connect') {
        $check = $pdo->prepare("SELECT COUNT(*) FROM startup_advisors WHERE startup_id = ? AND advisor_id = ?");
        $check->execute([$userId, $advisorId]);
        $alreadyConnected = $check->fetchColumn();

        if (!$alreadyConnected) {
            $connect = $pdo->prepare("INSERT INTO startup_advisors (startup_id, advisor_id) VALUES (?, ?)");
        ...
        $delete = $pdo->prepare("DELETE FROM startup_advisors WHERE startup_id = ? AND advisor_id = ?");
...
$stmt = $pdo->prepare("SELECT advisor_id FROM startup_advisors WHERE startup_id = ?");
$stmt->execute([$userId]);
