...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['service_id'], $_POST['card_number'])) {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM startup_services WHERE startup_id = ? AND service_id = ?");
    ...
    if (!$alreadySubscribed) {
        ...
        // Insert into startup_services
        $insertService = $pdo->prepare("INSERT INTO startup_services (startup_id, service_id) VALUES (?, ?)");
...
// Fetch subscribed service IDs
$stmt = $pdo->prepare("SELECT service_id FROM startup_services WHERE startup_id = ?");
$stmt->execute([$userId]);
...
<main class="page-wrapper">
    <section style="max-width: 1100px; margin: auto; padding: 40px 20px; line-height: 1.6;">
        ...
        <div style="text-align: center;">
            <img src="assets/images/services.jpg" alt="Startup" style="width: 1100px; height: 420px; object-fit: cover; border-radius: 12px; max-width: 100%;">
        </div>
